...
class TrainGenerator(Sequence):
    def __init__(self, conf, train_df, train_dict, test_dict, batch_size):
        self.conf = conf
...
if __name__ == '__main__':
    ...
    pseudo_test_df = pseudo_test_df.loc[:, ['img_id', 'cls_id']]
    pseudo_test_df['istrain'] = np.zeros(pseudo_test_df.shape[0], dtype=np.uint8)
    ...
    for fold in range(car.conf.folds):
        ...
        train_df['istrain'] = np.ones(train_df.shape[0], dtype=np.uint8)
        train_df = pd.concat([train_df, pseudo_test_df], ignore_index=True)
        ...
        model.summary()
        model.fit_generator(generator=TrainGenerator(conf = car.conf, train_df = train_df, train_dict = train_dict, test_dict = test_dict, batch_size = car.conf.batch_size),
